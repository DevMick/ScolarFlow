-- Script de création des tables EduStats
-- (sans CREATE DATABASE - à exécuter après création de la base)
...
CREATE INDEX IF NOT EXISTS idx_statistics_config_user_id ON statistics_config(user_id);
...
DROP TRIGGER IF EXISTS update_statistics_config_updated_at ON statistics_config;
...
INSERT INTO classes (user_id, name, level, academic_year, description) 
SELECT 1, 'CM2-A', 'CM2', '2024-2025', 'Classe de CM2 - Section A'
WHERE NOT EXISTS (SELECT 1 FROM classes WHERE name = 'CM2-A' AND user_id = 1);

INSERT INTO classes (user_id, name, level, academic_year, description) 
SELECT 1, 'CE2-B', 'CE2', '2024-2025', 'Classe de CE2 - Section B'
WHERE NOT EXISTS (SELECT 1 FROM classes WHERE name = 'CE2-B' AND user_id = 1);

INSERT INTO classes (user_id, name, level, academic_year, description) 
SELECT 1, 'CM1-A', 'CM1', '2024-2025', 'Classe de CM1 - Section A'
WHERE NOT EXISTS (SELECT 1 FROM classes WHERE name = 'CM1-A' AND user_id = 1);

-- Confirmer que tout est créé
SELECT 'Base de données EduStats configurée avec succès!' as message;
